...
export const SidebarMenuItem = ({ item, collapsed, onSelect }) => (
  ...
  >
    ...
    <span
      className={`overflow-hidden whitespace-nowrap text-[13px] font-semibold tracking-[.01em] transition-all ${collapsed ? "w-0 opacity-0" : "w-auto opacity-100"}`}
    >
      {item.label}
    </span>
    {item.badge && !collapsed && (
      <span className="ml-auto rounded-full bg-accent/15 px-2 py-0.5 text-[9px] font-bold text-accent-soft">
        {item.badge}
      </span>
    )}
  </NavLink>
