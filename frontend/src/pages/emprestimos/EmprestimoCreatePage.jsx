...
export const EmprestimoCreatePage = () => {
  ...
  const confirm = async () => { setSubmitting(true); setSubmitError(""); try { const loan = await createEmprestimo(values, user); navigate("/emprestimos", { replace: true, state: { success: `Empréstimo de ${loan.patrimonio.nome} registrado com sucesso. O patrimônio agora está como Emprestado.` } }); } catch (error) { setSubmitError(error.message); setSubmitting(false); } };
