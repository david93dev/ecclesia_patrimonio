...
export const AuthLogin = () => {
  ...
  return (
    <main className="grid h-dvh max-h-dvh min-h-0 grid-cols-1 overflow-hidden lg:grid-cols-[minmax(430px,.94fr)_minmax(520px,1.06fr)]">
      <section aria-label="Apresentação do sistema" className="relative hidden h-dvh min-h-0 flex-col overflow-hidden bg-auth-background px-[clamp(42px,5vw,76px)] pt-10.5 pb-8.5 text-white lg:flex [@media(max-height:760px)]:pt-7 [@media(max-height:760px)]:pb-6">
        ...
        <div className="relative z-1 my-auto pt-10 [@media(max-height:760px)]:pt-4">
          <span className="inline-flex items-center gap-2 text-[10px] font-bold tracking-[1.45px] text-auth-heading-muted uppercase before:h-px before:w-5.5 before:bg-accent">Gestão inteligente de patrimônio</span>
          <h1 className="my-5 max-w-142.5 font-[Manrope] text-[clamp(36px,3.8vw,56px)] leading-[1.1] font-bold tracking-[-2.4px]">
            Cuide do que conta<br />a sua <em className="font-normal text-brand-300 not-italic">história.</em>
          </h1>
          <p className="m-0 max-w-117.5 text-sm leading-[1.7] text-auth-copy">Organize, preserve e acompanhe todo o patrimônio da sua comunidade em um só lugar.</p>
          ...
          <div className="relative mt-8.5 h-61.25 [@media(max-height:760px)]:mt-2 [@media(max-height:760px)]:h-42" aria-hidden="true">
            ...
            <img src={heroImage} alt="" className="absolute top-[47%] left-1/2 z-2 w-57.5 -translate-x-1/2 -translate-y-1/2 drop-shadow-hero" />
          </div>
        </div>
        <p className="relative z-1 m-0 text-[11px] tracking-[.7px] text-auth-footer">Tecnologia a serviço do seu legado.</p>
      </section>
      ...
      <section className="relative flex h-dvh min-h-0 items-start justify-center overflow-hidden bg-auth-surface-glow bg-app-background px-7 pt-29 pb-20.5 sm:items-center sm:px-[clamp(40px,7vw,120px)] sm:py-18 [@media(max-height:700px)]:pt-21 [@media(max-height:700px)]:pb-12">
        ...
        <div className="w-full max-w-107.5">
          <div className="mb-7.5 sm:mb-9.5 [@media(max-height:700px)]:mb-5">
            <h2 className="mt-4.5 mb-2 font-[Manrope] text-[27px] leading-[1.2] font-bold tracking-[-1.2px] text-foreground sm:text-[32px]">Bem-vindo de volta</h2>
            <p className="m-0 text-sm text-muted">Entre com seus dados para acessar sua conta.</p>
          </div>
          ...
          <div className="mt-8 border-t border-border-input pt-6.25 text-center text-[11px] text-muted [@media(max-height:700px)]:mt-4 [@media(max-height:700px)]:pt-4">
            <span>Precisa de ajuda?</span>{" "}<a href="mailto:suporte@ecclesia.com.br" className="font-bold text-brand-700">Fale com o suporte</a>
          </div>
        </div>
        ...
        <footer className="absolute right-7 bottom-7 left-7 flex justify-center text-[10px] text-muted-faint sm:right-[clamp(40px,7vw,100px)] sm:left-[clamp(40px,7vw,100px)] sm:justify-between [@media(max-height:700px)]:bottom-3">
          ...
          <nav aria-label="Links legais" className="hidden gap-5.5 sm:flex"><a className="hover:text-brand-700" href="#privacidade">Privacidade</a><a className="hover:text-brand-700" href="#termos">Termos de uso</a></nav>
        </footer>
      </section>
    </main>
  );
