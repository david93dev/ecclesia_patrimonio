...
const readLocal = () => {
  try {
    ...
    return [...custom, ...MOCK_DEPARTMENTS.filter((item) => !ids.has(item.id))];
  } catch { return MOCK_DEPARTMENTS; }
};
...
export const listDepartments = async () => { if (API_URL) return request("/departamentos"); await wait(); return readLocal(); };
export const getDepartment = async (id) => { if (API_URL) return request(`/departamentos/${id}`); await wait(); return readLocal().find((item) => String(item.id) === String(id)) ?? null; };
...
export const updateDepartment = async (id, data, user) => {
  ...
  const changes = ["nome", "descricao", "lider"].filter((field) => String(current[field] ?? "") !== String(data[field] ?? "")).map((field) => ({ campo: field, anterior: current[field] ?? "", atual: data[field] ?? "" }));
  const updated = { ...current, ...data, atualizadoEm: now, atualizadoPor: user?.name ?? "Usuário não identificado", historico: [...(current.historico ?? []), { data: now, acao: "Departamento atualizado", usuario: user?.name, alteracoes: changes }] };
...
export const deleteDepartment = async (id) => {
  ...
  if (assets.some((item) => normalize(item.departamento) === normalize(department.nome))) throw new Error("Não foi possível excluir este departamento. Existem patrimônios vinculados a ele. Para realizar a exclusão, primeiro é necessário desvincular esses patrimônios.");
